...
{
  // if both lists are empty return and pop run_time stack
   if(xHead == 0 && yHead == 0)
    return;

    // else if both lists contain elements proceed normally
     ...
     // else find which list still has elements in it.
      else
      {
        if(xHead == 0)
        {
   ...
   // incriment zHead to the next Null node to insert next node.
   // call function again.
