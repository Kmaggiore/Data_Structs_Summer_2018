...
namespace CS3358_FA2017
{
   ...
   : used(src.used), current_index(src.current_index), capacity(src.capacity)
   {
     data = new value_type [capacity];

     memcpy(data, src.data, sizeof(value_type) * src.used);
   }
   ...
   void sequence::resize(size_type new_capacity)
   {
      ...
      memcpy(newData,data,sizeof(value_type)*used);
   ...
   void sequence::remove_current(void)
   {
    if(is_item()){
   ...
   sequence& sequence::operator=(const sequence& rhs)
   {
      ...
      if(used < rhs.capacity)
        resize(rhs.capacity);

      if(this != &rhs){

        value_type *newData = new value_type [rhs.capacity];
